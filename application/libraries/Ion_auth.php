...
class Ion_auth
{
	...
	public function user_group()
	{
		...
		if (!isset($group_id)) 
		{
			$fields = "users_groups.group_id AS group_id";
			$table  = 'users';
			$tbl_join = 'users_groups';
			$join_col = "users.id = users_groups.user_id";

			$user_id = array('users_groups.user_id' => $this->session->userdata('user_id')); 

			$result  = $this->common_model->DJoin($fields,$table,$tbl_join,$join_col,'',$user_id);
			...
			{
				$sess_arr = array(
								'user_group_id' => $result[0]->group_id,
							);
